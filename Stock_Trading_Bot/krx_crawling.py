ver = "#version 1.0.1"
print(f"krx_crawling Version: {ver}")
...
class KRXCrawler:
    def __init__(self, snapshot_dir_name='krx_snapshots', download_dir_name='KRX_xls'):
        db_url = URL(
    ...
    def variable_setting(self):
        self.FNAME_PATTERN = 'data*.xls'
        #'투자??종목*.xls'
        # 2007년 이전에는 kind 상에 데이터 없다.
        # 크롤링 시작일
        self.DEFAULT_START_DATE = datetime.date(2001, 1, 1)
    ...
    def craw(self):
        ...
        driver_path = pathlib.Path(__file__).parent.absolute() / 'drivers'
        self.driver = webdriver.Chrome(executable_path=ChromeDriverManager(path=driver_path).install(), options=options)
