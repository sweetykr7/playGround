...
class open_api(QAxWidget):
    def __init__(self):
        ...
        #조건 관련
        self.condition_count=60 # 1분에 한번만 불러오려고 이렇게 만듬.
        self.condition_status = 1 # 매수상태, 초기에는 매수상태로 함.

        ####################### 1.조건검색 변수설정 ##################################
        self.condition_scrNo = "1004"  # 조건검색 화면번호
        self.condition_name = "test01"  # 조건검색할 조건검색식 이름을 여기에 넣는다.
        self.condition_name2 = "test02"
        ...
        #self._set_signal_slots_realtime_condition() #실시간 조건검색
        self.comm_connect()
    ...
    def set_invest_unit(self):
        ...
        self.invest_unit = int(float(self.total_invest) * (1 / self.sf.divide_invest_unit) )
    ...
    def _set_signal_slots(self):
        try:
            ...
            self.OnReceiveConditionVer.connect(self.condition_slot_for_buy1)  # 조건검색식 불러오는 이벤트
            self.OnReceiveTrCondition.connect(self.condition_tr_slot_for_realtimetrade_for_buy1)  # 조건검색한 종목리스트 불러오는 이벤트
            #self.OnReceiveRealCondition.connect(self.condition_realtime)
            ############################################################################################
    ...
    def _set_signal_slots_realtime_condition(self):
        self.OnReceiveRealData.connect(self.condition_realtime())
    ...
    def init_db_setting_data(self):
        ...
        df_setting_data_temp = {'loan_money': [], 'limit_money': [], 'invest_unit': [], 'max_invest_unit': [],
                                ...
                                'min_crawler':[],'daily_buy_list': [],'monthly_gpa_crawling':[],'daily_sector_crawler': [],
    ...
    def init_db_t_sector(self):
        ...
        #  추가하면 여기에도 추가해야함
        df_t_sector_temp = {'date': [], 'check_item': [], 'code': [], 'code_name': [], 'd1_diff_rate': [], 'close': [],
                            'open': [], 'high': [], 'low': [], 'volume': [],
                            'clo5': [], 'clo10': [], 'clo20': [], 'clo40': [], 'clo60': [], 'clo80': [], 'clo100': [],
                            'clo120': [],
                            ...
                            'vol5': [], 'vol10': [], 'vol20': [], 'vol40': [], 'vol60': [], 'vol80': [], 'vol100': [],
                            'vol120': []}
    ...
    def get_today_buy_list(self):
        ...
        ############여기에 get_condition 추가############
        #아직까지 시간 몰라서 1분에 한번하게 변경

        if self.condition_count>60 and self.sf.condition_trade :

            self.get_condition_for_buy1()

            self.condition_count=0


        self.condition_count+=1
        logger.debug("self.condition_count : %s",self.condition_count)
...
#################################### 3.조건검색 부분 #########################################################
    ...
    def get_condition_for_buy1(self):
        logger.debug("Get_condition_for_buy1 pass")
        self.dynamicCall("GetConditionLoad()")  # 조건검색식 목록을 요청하는 함수
        self.condition_event_loop = QEventLoop()
        self.condition_event_loop.exec_()
    ...
    def condition_slot_for_buy1(self, lRet, sMsg):
        if lRet == 1:
            logger.debug("조건검색 호출처리: %s", sMsg)
            condition_list = self.dynamicCall("GetConditionNameList()")  # 조건검색식 목록 불러오는 함수
            condition_list = condition_list.split(';')[:-1]
            ...
            for l in condition_list:
                condition_list_index = l.split('^')[0]
                condition_name = l.split('^')[1]
                if condition_name == self.condition_name and self.condition_status == 1:
                    ...
                    if answer == 1:
                        ...
                        logger.debug("%s 조건검색요청처리: 실패", condition_name)
                elif condition_name == self.condition_name2 and self.condition_status == 2:
    ...
    def condition_tr_slot_for_realtimetrade_for_buy1(self, sScrNo, strCodeList, strConditionName, nIndex, nNext):
        logger.debug("condition_tr 들어옴======================")
        stock_code_list = strCodeList.split(';')[:-1]
        logger.debug("조건검색종목리스트: %s개", len(stock_code_list))

        if self.condition_status ==1:
            self.sf.realtime_daily_buy_list_condition = stock_code_list
            #print("통과 확인1")
            #self.sf.get_date_for_simul()
            #print("======================self.sf.get_date_for_simul()확인",self.sf.date_rows[-1][0])
            #print("통과 확인2")

            #print(self.sf.date_rows)
            ...
            #change하려면 아래에서 바꿔야됨
            #self.condition_status = 2  # 1이 매수
            self.condition_event_loop.exit()
            print("매수 통과 확인=========================")
        elif self.condition_status ==2:
            self.sf.sell_list_condition = stock_code_list

            self.condition_status = 2  # 2는 매도
            self.condition_event_loop.exit()
            print("매도 통과 확인=========================")

            #sell list관련, 매도할 대상들에 대한 식을 여기에 넣자.


        self.condition_event_loop.exit()

    def condition_realtime(self, sCode, sRealType, sRealData):
        logger.debug("real_condition 들어옴===========")
        self.condition_event_loop.exit()
