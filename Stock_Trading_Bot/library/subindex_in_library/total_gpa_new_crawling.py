...
class GPACrawl:
    ...
    def get_data_info(self, cmp_code):
        ...
        try:
            return pd.read_html(res.text)[1],pd.read_html(res.text)[3]
    ...
    def get_latest_index(self):
        ...
        self.today_month = datetime.datetime.today().strftime("%Y%m%d")
        try:
            latest_code = self.db_engine.execute("""
                        SELECT code FROM total_gpa_new 
                        WHERE collecting_month = {}
                        ORDER BY code DESC LIMIT 1
                    """.format(self.today_month)).first()[0]
    ...
    def is_data_exist(self,code,qq):
        try:
            sql=f'''
                    SELECT * FROM total_gpa_new 
                            WHERE year_qq = {qq}
                            and code={code}
                '''
            is_data_exist=self.db_engine.execute(sql).fetchall()
        except:
            is_data_exist=[]
        return len(is_data_exist)
    ...
    def crawl(self):
        ...
        for stock_code in self.stock_item_all[latest_index:]:
           ...
           # print(df)
            ...
            years=[y.split('/')[0] for y in year_df.columns if not y.startswith('Unnamed:')]
            year_qq = [y.replace('/','') for y in year_df.columns if not y.startswith('Unnamed:')]
            #last_year = years[-1]
            #last_qq=year_df.columns[-1].replace('/','')


            new_df = pd.DataFrame(columns=['code', 'code_name','collecting_month','year','year_qq', 'gp', 'a'])
            ...
            for k in range(len(years)):
                test1=0
                if self.is_data_exist(code,year_qq[k]) ==0 :
                    test = 0
                    new_df.loc[len(new_df)] = (code, code_name, self.today_month, years[k],year_qq[k], df.iloc[target_line,k+1],df2.iloc[target_line2,k+1])  # loc
            if len(new_df) >0:
                new_df.to_sql('total_gpa_new', self.db_engine, if_exists='append', index=False)
