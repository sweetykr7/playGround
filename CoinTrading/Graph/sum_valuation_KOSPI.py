...
def get_factors(s_date, s_name):
    ...
    if type(s_name) == str:
        ...
        firstdate = datetime.strptime(sec.iloc[0][0], "%Y%m%d")
        lastdate = datetime.strptime(rows[-1][0], "%Y%m%d")

        date_diff = lastdate - firstdate

        cagr = getCAGR(sec.iloc[0][1], sec.iloc[0][-1], (date_diff.days / 365))


        return [sec, sec_dpc_cs, cagr]
        # return [sec, sec_diff]
        ...
        print(f"{s_name}, cagr : {cagr}, days : {date_diff.days}, years : {date_diff.days/365}, total : {simul.iloc[-1][1] + pre_invest}")

        return [simul, simul_dpc_cs, cagr]
...
class PofitGraph:
    ...
    def __init__(self):
        ...
        #!@
        simul_num = [1402,'KOSPI']
        #293,299,402,514,515,516,517 일별 시뮬
        # 분별 시뮬
    ...
    #################################### 그래프 그리기 ##############################################
    def get_graph(self, tname, tname_dcp_cs, tlabel, cagr):
        ...
        self.ax.set_title('★★★일목균형표로 매수/거래대금별 매수/리밸런싱(3개월)/★★★\n'
                          '일목균형표/초기투자금 1000만원/손절(-40%)/'
                          '20분할 매수/\n'
                          '>>293:선행스팬1~2사이 매수 10분할, 299:293의 15분할, 402:선행스팬1,2평균까지만 매수 둘다 10분할\n'
                          '(293Base)514:익절10%, 515:익절15% (20분할)\n'
                          '(402Base)516:익절10%, 517:익절15% (20분할)'
                          ) #None
        #238 20%, 240 30%
    ...
    def get_all_profit_graphs(self, s_date, s_num):
        ...
        for s_name in s_num:
            [tname, tname_dcp_cs, cagr] = get_factors(s_date, s_name)
            self.get_graph(tname, tname_dcp_cs, s_name, cagr)
