...
#from telegram_bot import Bot_Alarm_System as chatbot
import sys
...
class Trader(QMainWindow):
    def __init__(self):
        ...
        #self.chatbot=chatbot.bot_telegram_class()
        # self.chatbot_selllist=[]
        # self.chatbot_sell_break = 0
        #
        # if self.open_api.trader_start_check()!=self.open_api.today:
        #     self.chatbot.send_text(f'''Joey Desktop의 Bot2 Trader Algo{cf.real_simul_num} Trader가 시작되었습니다.''')
        #     self.open_api.trader_start_update()
    ...
    def get_sell_list_trade(self):
        ...
        #self.open_api.chegyul_check() #일단은 주식과 달라서 꺼줬음. 향후에 추가해야될지도 모름.
    ...
    def auto_trade_sell_stock(self):
        ...
        for i in range(len(self.sell_list)):
            ...
            # 텔레그램 보내는 곳임, 일단 무시
            # for sell_list in self.chatbot_selllist:
            #     if sell_list == str(get_sell_code):
            #         self.chatbot_sell_break = 1
            # if QTime.currentTime() >= QTime(9, 0, 0) and self.chatbot_sell_break==0 :
            #     self.chatbot.sell_alarm(str(get_sell_code),str(get_sell_rate),str(get_sell_num) )
            #     self.chatbot_selllist.append(get_sell_code)

            if get_sell_code != False and get_sell_code != "0" and get_sell_code != 0:
                if get_sell_rate < 0:
                    logger.debug("손절!!!!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ " + str(
                        get_sell_code) + " $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
                    self.open_api.sell_order(get_sell_code,get_sell_num)
    ...
    def market_time_check(self):
        ...
        if self.current_time > self.market_start_time and self.current_time < self.market_end_time:
            logger.debug('''Setting된 투자 가능한 시간입니다.
                            만약 투자를 하고 싶지 않다면 trader.py 파일의 variable_setting 함수에서 시간을 변경해주세요''')
            return True
        else:
            logger.debug('''Setting된 투자 시간이 아닙니다.
                            만약 투자를 하고 싶다면 trader.py 파일의 variable_setting 함수에서 시간을 변경해주세요''')
            return False
